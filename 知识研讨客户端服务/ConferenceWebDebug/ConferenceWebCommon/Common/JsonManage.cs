...
namespace ConferenceWebCommon.Common
{
    public class JsonManage
    {
        ...
        /// <returns></returns>
        public static List<T> JsonToEntity<T,P,H>(string json, char split)
        {
                            ...
                            foreach (var dicChild in dicList)
                            {
                                foreach (var property in propertyArray)
                                {
                                    if (property.Name.Equals(dicChild.Key))
                                    {
                                        ...
                                        {
                                            int newVlaue = Convert.ToInt32(dicChild.Value);
                                            property.SetValue(obj, newVlaue, null);
                                        }
                                        else if (property.PropertyType == typeof(DateTime))
                                        {
                                            DateTime newVlaue = Convert.ToDateTime(dicChild.Value);
                                            property.SetValue(obj, newVlaue, null);
                                        }
                                        else if (property.PropertyType == typeof(List<string>))
                                        {
                                            string[] array = Convert.ToString(dicChild.Value).Split(new char[] { split });
                                            List<string> newVlaue = array.ToList<string>();
                                            property.SetValue(obj, newVlaue, null);
                                        }
                                        else if (property.PropertyType == typeof(Uri))
                                        {
                                            Uri newVlaue = new Uri(Convert.ToString(dicChild.Value), UriKind.RelativeOrAbsolute);
                                            property.SetValue(obj, newVlaue, null);
                                        }
                                        else if (property.PropertyType == typeof(P))
                                        {
                                            int typeValue =0;
                                            string strValue = Convert.ToString(dicChild.Value);
                                           bool result = int.TryParse(strValue, out  typeValue);
                                            if(result)
                                            {
                                                P pValue = (P)Enum.Parse(typeof(P), strValue);
                                                property.SetValue(obj, pValue, null);
                                            }

                                        }
                                        else if (property.PropertyType == typeof(H))
                                        {
                                            int typeValue = 0;
                                            string strValue = Convert.ToString(dicChild.Value);
                                            bool result = int.TryParse(strValue, out  typeValue);
                                            if (result)
                                            {
                                                H pValue = (H)Enum.Parse(typeof(H), strValue);
                                                property.SetValue(obj, pValue, null);
                                            }

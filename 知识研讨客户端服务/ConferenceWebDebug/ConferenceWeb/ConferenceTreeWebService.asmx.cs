...
namespace ConferenceWeb
{
    ...
    public class ConferenceTreeWebService : System.Web.Services.WebService
    {
        ...
        /// 后续进入的成员进行一次初始化的加载
        /// </summary>
        public static Dictionary<int, ConferenceTreeInitRefleshEntity> ConferenceTreeInitRefleshEntity_dic = new Dictionary<int, ConferenceTreeInitRefleshEntity>();
        ...
        [WebMethod]
        public ConferenceTreeInitRefleshEntity GetAll(int conferenceID)
        {
            //上锁,达到线程互斥作用
            //lock (objGetAll)
            //{
                ...
                try
                {
                    ...
                    {
                        //包含该会议
                        if (ConferenceTreeInitRefleshEntity_dic.ContainsKey(conferenceID))
                        {
                            //获取会议的树信息
                            initRefleshEntity = ConferenceTreeInitRefleshEntity_dic[conferenceID];
                        }
                        else
                        {
                            //生成会议树的源数据
                            ...
                            //字典添加树视图的源数据
                            ConferenceTreeInitRefleshEntity_dic.Add(conferenceID, initRefleshEntity);
                        }
                    }
                }
                catch (Exception ex)
        ...
        [WebMethod]
        public void UpdateOne(int conferenceID, ConferenceTreeItemTransferEntity academicReviewItemTransferEntity)
        {
            //上锁,达到线程互斥作用
            lock (objUpdateOne)
            {
        ...
        [WebMethod]
        public void UpdateTittle(int conferenceID, ConferenceTreeItemTransferEntity academicReviewItemTransferEntity)
        {
            //上锁,达到线程互斥作用
        ...
        [WebMethod]
        public void LinkAdd(int conferenceID, ConferenceTreeItemTransferEntity academicReviewItemTransferEntity)
        {
            //上锁,达到线程互斥作用
            lock (objLink)
            {
                ...
                try
                {
                    ...
                    for (int i = 0; i < treeList.Count; i++)
                    {
                        ...
                        if (item.Guid.Equals(beforeItemGuid))
                        {
                            ...
                            this.InformClient(conferenceID, beforeItemGuid, nowItemGuid);
                            break;
                        }
                    }
                }
        ...
        [WebMethod]
        public void VoteChanged(int conferenceID, ConferenceTreeItemTransferEntity academicReviewItemTransferEntity, string Participant)
        {
            //上锁,达到线程互斥作用
            lock (objVoteChanged)
            {
        ...
        /// <param name="academicReviewItemTransferEntity">知识树映射实体</param>
        [WebMethod]
        public void AddOne(int conferenceID, ConferenceTreeItemTransferEntity academicReviewItemTransferEntity)
        {
            //上锁,达到线程互斥作用
            lock (objAddOne)
            {
        ...
        /// <param name="academicReviewItemTransferEntity">知识树映射实体</param>
        [WebMethod]
        public void DeleteOne(int conferenceID, ConferenceTreeItemTransferEntity academicReviewItemTransferEntity)
        {
            //上锁,达到线程互斥作用
            lock (objDeleteOne)
            {
        ...
        [WebMethod]
        public void SetAll(int conferenceID, ConferenceTreeInitRefleshEntity conferenceTreeInitRefleshEntity)
        {
            //上锁,达到线程互斥作用
            lock (objSetAll)
            {
                try
                {
                    ...
                    {
                        //移除当前会议的所有节点
                        ConferenceTreeInitRefleshEntity_dic.Remove(conferenceID);
                        //刷新
                        ConferenceTreeInitRefleshEntity_dic.Add(conferenceID, conferenceTreeInitRefleshEntity);
                        //操作类型为刷新
                        ...
                        //实时同步(刷新所有)
                        InformClient(conferenceID, new ConferenceTreeItemTransferEntity() { Operation = ConferenceTreeOperationType.RefleshAllType });
                    }
                }
                catch (Exception ex)
                {
        ...
        /// </summary>
        /// <param name="conferenceName">会议名称</param>
        public void InformClient(int conferenceID, ConferenceTreeItemTransferEntity conferenceTreeItemTransferEntity)
        {
            try
            {
                if (conferenceID!=0)
                {
                    ...
                    {
                        ConferenceClientAcceptType = ConferenceClientAcceptType.ConferenceTree,
                    ...
                    //会议通讯节点信息发送管理中心
                    Constant.SendClientCenterManage(Constant.DicTreeMeetServerSocket, conferenceID, pack);
                }
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <param name="conferenceName">会议名称</param>
        public void InformClient(int conferenceID, int beforeGuid, int nowGuid)
        {
            try
            {
                if (conferenceID!=0)

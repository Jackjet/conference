    ...
    public partial class App : Application
    {
        ...
        public App()
        {
            try
            {
                ...
                //登录初始化配置2
                this.LoginEnviromentInit_Model();
            }
            catch (Exception ex)
            {
            ...
            try
            {
                this.Dispatcher.BeginInvoke(new Action(() =>
                {
                    if (this.loginWindow != null)
                    {
                        ...
                        this.loginWindow.LoginPanelIsEnable = true;
                    }
                }));
            }
            catch (Exception ex)
        ...
        private void Current_Exit_Help()
        {
            try
            {
                if (MainPage.mainPage != null)
                {
                    ...
                    //释放之前使用的通讯节点
                    MainPage.mainPage.DisPoseServerSocketArray(Constant.ConferenceName, new Action<bool>((isSuccessed) =>
                    {
                    }));

                    //工具箱窗体关闭
                    if (MainPage.mainPage.ToolCmWindow != null)
                    {
                        MainPage.mainPage.ToolCmWindow.Close();
                    }
                }
                //Thread.Sleep(1000);
                GC.Collect();

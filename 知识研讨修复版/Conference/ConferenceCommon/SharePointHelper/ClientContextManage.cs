...
namespace ConferenceCommon.SharePointHelper
{
    ...
    public class ClientContextManage
    {
        ...
        {
            List<SP.Folder> fileList = new List<SP.Folder>();
                ...
                if (ClientContext != null)
                {
                    ...
                    using (ClientContext)
                    {
                        ...
                        foreach (SP.Folder item in documentsList.RootFolder.Folders)
                        {
                            ...
                            ////加载文档创建者
                            //LoadMethod(item.Files);
                            ////加载文档修改者
                            //LoadMethod(item.Folders);
                            //LoadMethod(item);
                            //加载文档
                            fileList.Add(item);
                        }
        ...
        /// <param name="siteUrl">站点地址</param>      
        /// <returns>返回的所有列表</returns>
        public SP.ListCollection GetAllLists2(string webSiteUrl, string userName, string passWord, string doMain)
        {
            SP.ListCollection listCollection = null;
            try
            {
        ...
        public string CreateFolder(SP.Folder folder, string folderName)
        {
            ...
            try
            {
                ...
                using (ClientContext)
                {
                    ...
                    folder.Folders.Add(folder.ServerRelativeUrl + "/" + folderName);
                }
            }
            catch (Exception ex)
            {
        ...
        /// 删除文件夹
        /// </summary>
        public void DeleteFolder(SP.Folder folder, string folderName)
        {
            try
            {
                //创建客户端对象模型
        ...
        public void UploadFileToFolder(SP.Folder folder, string fullName)
        {         
            try
            {
                if (ClientContext != null)
                {
                    ...
                    if (File.Exists(fullName))
                    {
                        ...
                        this.UploadFileToFolder(folder, fileName, bytes);
                    }
                }
            }
            catch (Exception ex)
        ...
        /// <param name="FileName"></param>
        /// <returns></returns>
        public string DeleFile(SP.Folder folder, string FileName)
        {
            string result = string.Empty;
            try
            {
       ...
       public  SP.ListItemCollection GetListItemCollectionFromSP(string documentListName, string name, string value, int rowLimit)
        {
            //声明一个item的集合
        ...
        public bool LoadMethod(SP.ClientObject obj)
        {
            bool result = false;
            try
            {

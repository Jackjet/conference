...
namespace ConferenceCommon.FileDownAndUp
{
    public class FtpManage
    {
        ...
        public void UploadFtp(string FileName, string newFileName, string ftpIp, string destFilePath, string ftpUser, string ftpPassword, Action<long, double> callback, Action<Exception, bool> compleateCallback)
        {
            try
            {
                Thread thread = new Thread(new ThreadStart(() =>
                {
                    ...
                    //string ftpIP = ftpRoot.Substring()

                    if (!string.IsNullOrEmpty(destFilePath))
                    {
                        FtpCheckDirectoryExist(ftpIp, ftpUser, ftpPassword, destFilePath + "/");
                    }
                    //GetFileList(ftpIp, destFilePath, ftpUser, ftpPassword, ((fileLilllst, error) =>
                    //    {
                    //        if (fileLilllst != null)
                    //        {
                    //            if (fileLilllst.Contains(newFileName))
                    //            {
                    //                compleateCallback(new Exception("你所上传的目录里已经包含同名称的文件,请修改名称后再上传"), false);
                    //                return;
                    //            }
                    //        }
                            _reqFTPUp = (FtpWebRequest)FtpWebRequest.Create(new Uri(uri));
        ...
        public static void GetFileList(string ftpIp, string destFilePath, string ftpUser, string ftpPwd, Action<string[], string> callBack)
        {
            ...
            try
            {
                GetFileListCallBack = new Thread(() =>
                {
                    ...
                    reqFTP =
                        (FtpWebRequest)FtpWebRequest.Create(new Uri(ftpIp + destFilePath));

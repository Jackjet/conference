...
namespace ConferenceModel.Model
{
    public class ConferenceInfo
    {
        ...
        {
            get { return ConferenceInfo.client; }
        }
        ...
        /// 更改会议信息（模式切换）
        /// </summary>
        Action<bool> UpdateConferenceInformation_CallBack = null;
        ...
        internal void ConferenceInfoInit(BasicHttpBinding binding, EndpointAddress endpoint)
        {
            try
            {
                if (ConferenceInfo.client == null)
                {
                    ...
                    //更改客户端模式
                    ConferenceInfo.Client.UpdateTempInformationCompleted += Client_UpdateTempInformationCompleted;
                    //客户端统一命令
        ...
        public void ChangeClientModel(string conferenceName, bool isSimpleModel, bool isEducationModel, Action<bool> callBack)
        {
            try
            {
                if (ConferenceInfo.client != null)
                {
                    //模式切换回调
                    this.UpdateConferenceInformation_CallBack = callBack;
                    //异步获取会议信息
                    ConferenceInfo.Client.UpdateTempInformation(conferenceName, isSimpleModel, isEducationModel);
                }
            }
            catch (Exception ex)
            {
        ...
        void Client_UpdateTempInformationCompleted(object sender, System.ComponentModel.AsyncCompletedEventArgs e)
        {
            try
            {
                if (e.Error == null)
                {
                    if (this.UpdateConferenceInformation_CallBack != null)
                    {
                        this.UpdateConferenceInformation_CallBack(true);
                    }
                }
            }
            catch (Exception ex)
        ...
        /// <param name="callBack"></param>
        public void FillConferenceOfficeServiceData(string conferenceName, string sharer, ConferencePageType conferencePageType, Action<bool> callBack)
        {
            try
            {
                this.FillConferenceInfoServiceData_CallBack = callBack;

                ConferenceInfo.Client.FillSyncServiceDataAsync(conferenceName, sharer, conferencePageType);
            }
            catch (Exception ex)
            {
                LogManage.WriteLog(this.GetType(), ex);
        ...
        public void RemoveCurrentUser_AllClientSocket(string conferenceName, string contactUri, Action<bool> callback)
        {
            try
            {
                this.RemoveCurrentUser_AllClientSocket_CallBack = callback;
                ConferenceInfo.Client.RemoveAllClientSocketBySomeOneAsync(conferenceName, contactUri);
            }
            catch (Exception ex)
            {
                LogManage.WriteLog(this.GetType(), ex);

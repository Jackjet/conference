...
namespace ConferenceModel.Model
{
    public class ConferenceVersion
    {
        ...
        {
            get { return ConferenceVersion.client; }
        }
        ...
        internal void ConferenceVersionInit(BasicHttpBinding binding, EndpointAddress endpoint)
        {
            try
            {
                if (ConferenceVersion.client == null)
                {
                    ConferenceVersion.client = new ConferenceVersionWebserviceSoapClient(binding, endpoint);
                    ConferenceVersion.client.ClientCredentials.Windows.AllowedImpersonationLevel =
                       TokenImpersonationLevel.Impersonation;

                    ConferenceVersion.Client.NeedVersionUpdateCompleted += ConferenceVersionClient_NeedVersionUpdateCompleted;
                }
            }
            catch (Exception ex)
            {
        ...
        public void NeedVersionUpdate(string PCVersion, Action<bool, Exception> callBack)
        {
            try
            {
                ...
                ConferenceVersion.Client.NeedVersionUpdateAsync(PCVersion);
            }
            catch (Exception ex)
            {
                LogManage.WriteLog(this.GetType(), ex);

...
namespace ConferenceModel.Model
{
    public class ConferenceAudio
    {
        ...
        {
            get { return ConferenceAudio.client; }
        }
        ...
        internal void ConferenceAudioInit(BasicHttpBinding binding, EndpointAddress endpoint)
        {
            try
            {
                if (ConferenceAudio.client == null)
                {
                    //生成语音转文字
                    ConferenceAudio.client = new ConferenceAudioWebserviceSoapClient(binding, endpoint);
                    //生产凭据
                    ConferenceAudio.Client.ClientCredentials.Windows.AllowedImpersonationLevel =
                        TokenImpersonationLevel.Impersonation;
                    //添加节点完成事件
                    ConferenceAudio.Client.AddOneCompleted += Client_AddOneCompleted;
                    //删除语音子节点完成事件
                    ConferenceAudio.Client.DeleteOneCompleted += ConferenceAudioClient_DeleteOneCompleted;
                    //获取语音子节点完成事件
                    ConferenceAudio.Client.GetAllCompleted += ConferenceAudioClient_GetAllCompleted;
                    //获取语音子节点语音转文字信息完成事件
                    ConferenceAudio.Client.SettingAudioTransferTxtCompleted += client_SettingAudioTransferTxtCompleted;
                    //更新语音子节点完成事件
                    ConferenceAudio.Client.UpdateOneCompleted += client_UpdateOneCompleted;
                    //上传音频文件返回标示
                    ConferenceAudio.Client.NotifyAudioFileUploadCompleateCompleted += Client_NotifyAudioFileUploadCompleateCompleted;
                }
            }
            catch (Exception ex)
            {
        ...
        /// <param name="conferenceName"></param>
        /// <param name="callback"></param>
        public void GetAll(string conferenceName, Action<ConferenceAudioInitRefleshEntity> callback)
        {
            try
            {
                //设置回调
                ...
                //向服务器申请获取语音子节点
                ConferenceAudio.Client.GetAllAsync(conferenceName);
            }
            catch (Exception ex)
            {
                LogManage.WriteLog(this.GetType(), ex);
